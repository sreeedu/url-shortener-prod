...
from app.core.exceptions import (
    ...
    ResetTokenInvalidError, InvalidTokenError, UnverifiedEmailError,
)
...
async def signup(
    ...
    ver_token = create_verification_token(str(user.id))
...
async def verify_email(
    ...
    user_id_str = decode_token(payload.token, token_type="email_verify")
    if not user_id_str:
        raise InvalidTokenError()
    try:
        user_id = uuid.UUID(user_id_str)
...
async def resend_verification(
    ...
    return {"message": "If this account is unverified, a verification email has been sent."}
