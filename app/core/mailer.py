...
def send_admin_invite_email(to_email: str, inviter_email: str, raw_token: str) -> bool:
    invite_link = f"{settings.FRONTEND_URL}/#/accept-admin?token={raw_token}"
    msg = MIMEMultipart("alternative")
    msg["Subject"] = "You have been invited to become a Platform Admin"
    msg["From"] = f"{settings.EMAILS_FROM_NAME} <{settings.EMAILS_FROM_EMAIL}>"
    msg["To"] = to_email
    import uuid
    from email.utils import formatdate
...
{inviter_email} has invited you to become a Platform Admin on {settings.APP_NAME}.
...
"""
    html = f"""\
<html><body style="font-family:sans-serif;max-width:480px;margin:auto;padding:24px;">
<h2 style="color:#1F2937;">Admin Invitation</h2>
<p><strong>{inviter_email}</strong> has invited you to become a Platform Admin on {settings.APP_NAME}.</p>
<p>
  <a href="{invite_link}"
     style="background:#4F46E5;color:white;padding:12px 24px;
            border-radius:6px;text-decoration:none;display:inline-block;
...
def send_admin_invite_confirmation(to_email: str, invitee_email: str) -> bool:
    msg = MIMEMultipart("alternative")
    msg["Subject"] = "Admin Invitation Sent"
    msg["From"] = f"{settings.EMAILS_FROM_NAME} <{settings.EMAILS_FROM_EMAIL}>"
    msg["To"] = to_email
    import uuid
    from email.utils import formatdate
    ...
    text = f"Hi,\n\nYou have successfully sent a Platform Admin invitation to {invitee_email}.\n"
    html = f"<html><body style='font-family:sans-serif;'><h3>Invitation Sent</h3><p>You have successfully sent a Platform Admin invitation to <strong>{invitee_email}</strong>.</p></body></html>"
    msg.attach(MIMEText(text, "plain"))
    msg.attach(MIMEText(html, "html"))
...
<p>Welcome to {settings.APP_NAME}! Please verify your email address to get started.</p>
<p>
  <a href="{verify_link}"
     style="background:#4F46E5;color:white;padding:12px 24px;
            border-radius:6px;text-decoration:none;display:inline-block;
...
"""
    ...
    if not settings.SMTP_USER or not settings.SMTP_PASSWORD or not settings.EMAILS_FROM_EMAIL:
        logger.error("SMTP not configured for verification emails")
        return False
    try:
        context = ssl.create_default_context()
        with smtplib.SMTP(settings.SMTP_HOST, settings.SMTP_PORT, timeout=10) as server:
